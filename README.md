...
Vast is a framework for building RESTful backends using nothing but JSON files to define the endpoints, schemas and even the logic. The JSON format is similar to the Abstract Syntax Tree (AST) representation of a Typescript file, but simplified as much as possible without losing functionality.
